...
static struct kmem_cache *hk_range_node_cachep;
...
static int hk_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (sbi->cpus > POSSIBLE_MAX_CPU) {
		hk_err(sb, "hunter does't support more than "
			   __stringify(POSSIBLE_MAX_CPU) " cpus for now.\n");
		goto out;
	}
...
int hk_remount(struct super_block *sb, int *mntflags, char *data)
{
	...
	sb->s_flags = (sb->s_flags & ~MS_POSIXACL) |
				  ((sbi->s_mount_opt & HUNTER_MOUNT_POSIX_ACL) ?
				  MS_POSIXACL : 0);
