...
#define DECLARE_GENERIC_CACHEP(type, alloc_flags)                                             \
    ...
    {                                                                                         \
        ...
        if (strcmp(STRFY(type##_cachep), "obj_ref_inode_cachep") == 0) {                      \
            HK_STATS_ADD(ptl_mem_usage, sizeof(struct type));                                 \
        } else if (strcmp(STRFY(type##_cachep), "obj_ref_data_cachep") == 0) {                \
            HK_STATS_ADD(ptl_mem_usage, sizeof(struct type));                                 \
        } else if (strcmp(STRFY(type##_cachep), "obj_ref_attr_cachep") == 0) {                \
            HK_STATS_ADD(ptl_mem_usage, sizeof(struct type));                                 \
        } else if (strcmp(STRFY(type##_cachep), "obj_ref_dentry_cachep") == 0) {              \
            HK_STATS_ADD(ptl_mem_usage, sizeof(struct type));                                 \
        }                                                                                     \
        return p;                                                                             \
    }                                                                                         \
